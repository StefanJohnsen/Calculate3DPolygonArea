#pragma once
/*
  TriangulatePolygonEx.h
  
  C++ Template for triangulating a 3D polygon
  
  This template extends TriangulatePolygon.h by adding area
  calculation functions at the bottom of this file for calculating
  the area of a 3D polygon.
  
  Calculating the area of a polygon simply involves triangulating
  the polygon and accumulating the area of each triangle.
...
#ifndef TRIANGULATE_POLYGON_EX
#define TRIANGULATE_POLYGON_EX
...
namespace triangulate
{
	...
	template <typename P = Point, typename T = Triangle<P>>
	double calculateMeshArea(const std::vector<T>& mesh)
	{
		double area(0.0);
		...
		for( const auto& triangle : mesh )
		{
			...
			const auto a = triangulate::subtract(p1, p0);
			const auto b = triangulate::subtract(p2, p0);

			const auto c = triangulate::cross(a, b);

			area += triangulate::magnitude(c);
		}
	...
	template <typename P = Point, typename T = Triangle<P>>
	double calculateMeshArea(const std::vector<std::vector<T>>& meshes)
	{
		double area(0.0);

		for( const auto& mesh : meshes )
			area += triangulate::calculateMeshArea<P>(mesh);

		return area;
	}

	template <typename P = Point>
	double calculatePolygonArea(const std::vector<P>& polygon)
	{
		auto triangles = triangulate::triangulate<P, Triangle<P>>(polygon);

		return triangulate::calculateMeshArea<P, Triangle<P>>(triangles);
	}

	template <typename P = Point>
	double calculatePolygonArea(const std::vector<std::vector<P>>& polygons)
	{
		double area(0.0);

		for( const auto& polygon : polygons )
			area += triangulate::calculatePolygonArea<P>(polygon);

		return area;
	}
}

#endif // TRIANGULATE_POLYGON_EX
